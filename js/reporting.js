...
function renderWareneinsatzKachel() {
  ...
  if (rezepte.length === 0) {
    return tile(
      "Wareneinsatzquote",
      `<p class="empty-note">Noch kein Drink mit Verkaufspreis – Preise in der Karte eintragen, dann rechnet die Quote.</p>`,
      "menu-costing"
    );
  }
  ...
  rezepte.forEach((r) => {
    const kat = r.category || "Ohne Kategorie";
  ...
  function zeile(label, liste) {
    ...
    const trend =
      vorher === null
        ...
        : `<span class="report-trend-note">vor ${periodDays} Tagen: ${formatProzent(vorher)}</span>`;
    return `
  ...
  const gesamt = zeile("Gesamte Karte", rezepte);
  ...
  return tile(
    "Wareneinsatzquote",
    `${gesamt ?? ""}${proKategorie}
     <p class="report-tile-hint">Zielquote ${formatProzent(zielquote)} (aus der Kartenkalkulation). Rot heißt: darüber.</p>`,
    "menu-costing"
  );
}
...
function renderBestandswertKachel(punkte) {
  ...
  if (alleAbgeschlossen.length === 0) {
    return tile("Bestandswert", `<p class="empty-note">Noch keine abgeschlossene Inventur.</p>`, "inventory");
  }
  if (punkte.length === 0) {
    return tile(
      "Bestandswert",
      `<p class="empty-note">Keine abgeschlossene Inventur im gewählten Zeitraum.</p>`,
      "inventory"
    );
  }
  ...
  const hinweis =
    punkte.length === 1
      ? `<p class="report-tile-hint">Noch keine zweite Inventur im Zeitraum – Verlauf ab der nächsten.</p>`
      : "";

  return tile("Bestandswert", balkenHtml + hinweis, "inventory");
}
...
function renderAnsaetzeKachel() {
  ...
  if (preps.length === 0) {
    return tile("Ablaufende Ansätze", `<p class="empty-note">Noch keine Ansätze erfasst.</p>`, "preparations");
  }
  ...
  if (ablaufend.length === 0 && ueberfaellig.length === 0) {
    return tile(
      "Ablaufende Ansätze",
      `<p class="empty-note">Kein Ansatz läuft in den nächsten drei Tagen ab, und nichts ist im gewählten Zeitraum überfällig.</p>`,
      "preparations"
    );
  }
    ...
    const frist =
      tage < 0
        ...
        : tage === 0
          ...
          : tage === 1
            ? "läuft morgen ab"
            : `läuft in ${tage} Tagen ab`;
    return `<div class="report-list-row"><span>${escapeHtml(p.label)} <span class="report-tile-hint">(${escapeHtml(TYP_LABELS[p.prepType] ?? p.prepType)})</span></span><span class="menu-quote-high">${escapeHtml(frist)}</span></div>`;
  };

  const zeilen =
    ablaufend.map(zeile).join("") +
    (ueberfaellig.length > 0
      ? `<p class="report-tile-hint">Überfällig, noch nicht abgehakt (${periodDays} Tage zurück):</p>` +
        ueberfaellig.map(zeile).join("")
      : "");

  return tile("Ablaufende Ansätze", zeilen, "preparations");
}
...
function renderVerlusteKachel() {
  ...
  if (alle.length === 0) {
    return tile("Verluste nach Grund", `<p class="empty-note">Noch keine Verluste erfasst.</p>`, "losses");
  }
  ...
  if (imZeitraum.length === 0) {
    return tile(
      "Verluste nach Grund",
      `<p class="empty-note">Keine Verluste im gewählten Zeitraum (${periodDays} Tage).</p>`,
      "losses"
    );
  }
  ...
  const zeilen = zeilenDaten
    .map(([grund, d]) => {
      const hinweis =
        d.ohneWert > 0 ? ` <span class="report-tile-hint">(${d.ohneWert}× ohne berechenbaren Wert)</span>` : "";
      return `
        <div class="report-bar-row">
  ...
  return tile(`Verluste nach Grund (${periodDays} Tage)`, zeilen, "losses");
}
...
function renderPreissprungKachel() {
  ...
  if (alle.length === 0) {
    return tile(
      "Preissprünge",
      `<p class="empty-note">Kein Drink mit spürbarem Wareneinsatz-Anstieg seit dem letzten gespeicherten Preisstand.</p>`,
      "menu-costing"
    );
  }
  ...
  if (imZeitraum.length === 0) {
    return tile(
      "Preissprünge",
      `<p class="empty-note">Keine Preissprünge im gewählten Zeitraum (${periodDays} Tage) – ${alle.length} ältere Warnung(en) außerhalb.</p>`,
      "menu-costing"
    );
  }
  ...
  return tile(`Preissprünge (${periodDays} Tage)`, zeilen, "menu-costing");
}
...
async function renderAll() {
  ...
  gridEl.innerHTML =
    ...
    tile("Bestandswert", `<p class="empty-note">Lädt…</p>`, "inventory") +
